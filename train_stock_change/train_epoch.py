...
def load_data(path):
    ...
    try:
        loaded = np.load(path)
        return (
            torch.from_numpy(loaded["x_trains"]),
            torch.from_numpy(loaded["y_trains"]),
...
def training_loop(
    ...
    for epoch in range(n_epochs):
        ...
        batch_size = 256
        RMSE = eval_tune(
            device, model.state_dict(), num_in, num_layers, num_pred, hidden_size, evalnpz_path, batch_size
        )
        result = {
            "epoch": epoch,
...
def main(config):
    ...
    x_train, y_train, x_test, y_test, input_size = load_data("stock_data.npz")
    ...
    results = training_loop(
        ...
        config["num_pred"],
        config["hidden_size"],
        evalnpz_path='stock_data_eval.npz',
        optimizer=optimizer,
        loss_fn=loss_fn,
    ...
    model_name = f"ALL_p{config['num_pred']}_in{config['num_in']}_l{config['num_layers']}_h{config['hidden_size']}.pth"
...
def parse_args():
    ...
    if len(sys.argv) != 6:
        print(
            "Usage: python train.py <num_layers> <hidden_size> <n_epochs> <learning_rate> <dropout>"
        )
        sys.exit(1)
